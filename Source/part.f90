...
REAL(EB) :: PHI_RN,FLOW_RATE,THETA_RN,SPHI,CPHI,MASS_SUM,D_PRES_FACTOR, &
            ...
            VXMIN,VXMAX,VYMIN,VYMAX,VZMIN,VZMAX,VDX,VDY,VDZ,VEL_RAMP_FACTOR
...
SPRINKLER_INSERT_LOOP: DO KS=1,N_DEVC
   ...
   PARTICLE_INSERT_LOOP: DO I=1,N_INSERT
      ...
      CHOOSE_COORDS: DO
         ...
         IF (ABS(T_BEGIN-DV%T_CHANGE)<=TWENTY_EPSILON_EB .AND. PY%PARTICLE_VELOCITY_RAMP_INDEX>=1) THEN
             TSI = T
         ELSE
             TSI = T - DV%T_CHANGE
         ENDIF
         
         VEL_RAMP_FACTOR = 1._EB
         IF (PY%PARTICLE_VELOCITY_RAMP_INDEX > 0) THEN
            VEL_RAMP_FACTOR = EVALUATE_RAMP(TSI,PY%PARTICLE_VELOCITY_RAMP_INDEX)
         ENDIF
         PARTICLE_SPEED = PARTICLE_SPEED*VEL_RAMP_FACTOR
